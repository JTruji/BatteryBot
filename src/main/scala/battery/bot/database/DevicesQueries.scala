...
object DevicesQueries {
  ...
  def updateChargingTime(name: String, chargingTime: Double, userName: UUID): doobie.Update0 =
    sql"""update devices set charging_time = $chargingTime where name = $name and id_user = $userName""".update
